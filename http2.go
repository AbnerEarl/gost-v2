...
func (c *http2Connector) ConnectContext(ctx context.Context, conn net.Conn, network, address string, options ...ConnectOption) (net.Conn, error) {
	...
	if Debug {
		...
		log.Log("[http2]", conn.RemoteAddr(), conn.LocalAddr(), req.URL.String(), req.URL.Path, req.URL.RequestURI(), string(dump))
	}
...
func (tr *h2Transporter) Dial(addr string, options ...DialOption) (net.Conn, error) {
	...
	if Debug {
		dump, _ := httputil.DumpRequest(req, true)
		log.Log("[http2]", req.URL.String(), req.URL.Path, req.URL.RequestURI(), string(dump))
	}
	...
	if Debug {
		dump, _ := httputil.DumpResponse(resp, true)
		log.Log("[http2]", resp.Request.URL, resp.Request.RequestURI, req.URL.String(), req.URL.Path, req.URL.RequestURI(), string(dump))
	}
...
func (l *h2Listener) handleFunc(w http.ResponseWriter, r *http.Request) {
	...
	if Debug {
		dump, _ := httputil.DumpRequest(r, true)
		log.Log("[http2]", r.Method, r.URL.String(), r.URL.Path, r.URL.RequestURI(), string(dump))
	}
