...
// /Users/frank/go/pkg/mod/github.com/go-gost/x@v0.8.1/handler/http/handler.go
import (
	"bufio"
	"bytes"
	"context"
...
// SaveLargeStringToFile 高效写入大字符串
func SaveLargeStringToFile(filename string, content string, perm os.FileMode) error {
...
func (h *httpHandler) Handle(ctx context.Context, conn net.Conn, opts ...handler.HandleOption) (err error) {
	...
	// ctx = SaveRequestToContext(ctx, req)
	// if req != nil && req.Header != nil {
	// 	// 使用一个明确的 string key，避免 typed-key 的 cross-file 不一致问题
	// 	ctx = context.WithValue(ctx, "gost:original-request-headers", req.Header.Clone())
	// }
...
func (h *httpHandler) handleRequest(ctx context.Context, conn net.Conn, req *http.Request, ro *xrecorder.HandlerRecorderObject, log logger.Logger) error {
	...
	if u, _, _ := h.basicProxyAuth(req.Header.Get("Proxy-Authorization")); u != "" {
		fields["user"] = u
		ro.ClientID = u
		req.Header.Set("proxy_extra_info", u)
		log.Debug("set extra info .....")
	}
	log = log.WithFields(fields)

	// 在调用dial之前，将请求存入buffer ctx
	var buf bytes.Buffer
	ctx = ictx.ContextWithBuffer(ctx, &buf)
	// 同时将原始请求作为value保存
	ctx = context.WithValue(ctx, contextKeyRequest, req) // 直接保存，不依赖函数

	// 新增：把 headers 同步存入 ctx，便于 connector 读取
	if req != nil && req.Header != nil {
		ctx = context.WithValue(ctx, "gost:original-request-headers", req.Header.Clone())
	}
